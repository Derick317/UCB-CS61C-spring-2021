...
map:
    addi sp, sp, -24 # Fix BUG 3 (for saving t0, t1 and t2)
    sw ra, 0(sp)
    sw s1, 4(sp)
    sw s0, 8(sp)
...
mapLoop:
    lw t1, 0(s0)        # load the address of the array of current node into t1 # FIX BUG 5 (add t1, s0, x0)
    lw t2, 4(s0)        # load the size of the node's array into t2
   
    slli t3, t0, 2      # offset the array address by the count # FIX BUG 4 (add t1, t1, t0)
    add t1, t1, t3      
    lw a0, 0(t1)        # load the value at that address into a0
    
    sw t0, 12(sp)       # FIX BUG 3 
    sw t1, 16(sp)
    sw t2, 20(sp)
    
    jalr s1             # call the function on that value.
    
    lw t0, 12(sp)       # FIX BUG 3
    lw t1, 16(sp)
    lw t2, 20(sp)
    ...
    lw a0, 8(s0)        # load the address of the next node into a0 # FIX BYG 2 (la a0, 8(s0))
    add a1, x0, s1      # put the address of the function back into a1 to prepare for the recursion # FIX BUG 6 (lw a1, 0(s1))
